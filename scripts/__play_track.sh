...
make_session_name() {
    ...
    # 1. Replace non-alphanumeric/dash with underscore, lowercase, and truncate to 45 chars
    base="${song_title//[^[:alnum:]-]/_}"       # Replace unwanted chars with _
    base="${base,,}"                            # Lowercase
    base="${base:0:45}"                         # Truncate to 45 chars

    # 2. Short 4-char hash of the original title
    hash=$(printf '%s' "$song_title" | md5sum | cut -c1-4)
...
while true; do
	...
	if [[ "$RUN_MODE" == true ]]; then
      ...
      potential_session=$(make_session_name "$song_title")
...
if [[ -n $selected_track ]]; then
  ...
  tmux_session_name=$(make_session_name "$song_title")
